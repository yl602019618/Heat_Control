...
class ReplayBuffer(object):
    ...
    def sample_batch_FNO(self,batch_size,start = 0 , end = int(1e8),n_step = 2):
        ...
        '''
        idxs = torch.randint(start, min(self.size-n_step+1,end), size=(batch_size,))
        
        idxs = idxs.unsqueeze(-1)
        idx = idxs.clone()
        for i in range(n_step-1):
            idxs = torch.cat((idxs,idx+i+1),dim = 1)
        #obs = torch.zeros(batch_size*n_step,self.obs_dim , self.obs_dim)
        #act = torch.zeros(batch_size*(n_step-1),self.act_dim )
        #done = torch.zeros(batch_size*n_step , 1)
        idx_obs  = idxs.reshape(-1)
        idx_act = idxs[:,:-1].reshape(-1)
        obs = self.obs_buf[idx_obs].reshape(batch_size,n_step,self.obs_dim,self.obs_dim)
        act = self.act_buf[idx_act].reshape(batch_size , n_step-1,self.act_dim)
        rew=self.rew_buf[idx_act].reshape(batch_size, n_step-1,1)
        done=self.done_buf[idx_obs].reshape(batch_size,n_step, 1)
