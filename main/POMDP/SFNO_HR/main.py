...
class heat_eqn():
    def __init__(self,exp_step = 100, total_step = 10000,update_freq_model = 10 , update_freq_policy = 10,n_step = 3, device = None):
        ...
        self.obs_dim = [37,37]
        self.obs_dim_down = (self.obs_dim[0]-1)//4+1  # 16
        self.act_dim = 6 
        self.mode1 = 5
        self.mode2 = 5
        self.mode3 = 17
        ...
        self.width = 20
        self.device = device
        self.resolution = self.obs_dim[0]
        self.scale =4
         # for n_step = 5
    ...
    def exp_step(self):
        ...
        for n in range(self.exp_step_num):
            ...
            if n % 100 == 0:
                print('explore step = ', n)
            if done:
                self.env.reset()
                self.episode +=1
    
    # tools use for down sampling
    ...
    def loss_gen(self,output0,output1, truth, beta ,action):
        ...
        r = (output0.shape[1]-1)//(self.obs_dim_down -1)
        self.r = r
        #compute data_loss
    ...
    def train_SFNO(self,epoch,batch_size):
        ...
        self.grid = self.get_grid([batch_size,self.n_step-1,self.obs_dim_down,self.obs_dim_down]).to(self.device)
        ...
        for i in range(epoch):
            ...
            obs = self.down_sample(sample['obs']) #(batch_size,n_step ,grid_x,grid_y)
            input = obs[:,0:1,:,:]
            ...
            wandb.log({'loss_total': loss.item(), 'loss_data': self.data_loss.item(),'loss_boundary':self.boundary_loss.item(),'loss_physic':self.phy_loss.item(),'loss_bd_inside':self.boundary_loss.item()*self.beta_loss[1],'loss_physic_inside':self.phy_loss.item()*self.beta_loss[2]})
    ...
    def plot_result(self):
        n_step = self.n_step
        batch_size = 1
        self.grid = self.get_grid([batch_size,n_step-1,self.obs_dim_down,self.obs_dim_down])
        ...
        wandb.log({"error_tp1": wandb.Image('./img/error_bench_tp1.png')})
