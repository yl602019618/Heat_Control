...
class heat_eqn():
    def __init__(self,exp_step = 100, total_step = 10000,update_freq_model = 10 , update_freq_policy = 10,n_step = 3, device = None):
        ...
        self.data_real = ReplayBuffer( obs_dim = self.resolution, act_dim = self.act_dim, size =self.exp_step_num+1 ,device = device)
    ...
    def test_trajectory(self,test):
        ...
        length = 100
        if test  == False:
            self.data_test = self.data_real
        else :
            self.data_test = self.sample_traj(length)
        '''
        ...
        index = torch.arange(start =0,end = length - self.n_step) 
        ...
        input_HR = self.data_test.obs2_buf[indexs,:,:].reshape(-1,self.n_step-1,self.resolution,self.resolution)
        
        real_HR = self.data_test.obs_buf[indexs,:,:].reshape(-1,self.n_step-1,self.resolution,self.resolution) #batchsize , x,y
        output_HR = self.SFNO(self.downsample(input_HR).permute(0, 2, 3, 1)) # batchsize ,x,y,1
        print('task2 :output shape',output_HR.shape , 'input shape', input_HR.shape,'real_shape',real_HR.shape)

        HR_error = torch.norm(output_HR[:,:,:,0] - input_HR[:,-1,:,:], p =1 ,dim = [1,2])/self.resolution**2
        Base_error = torch.norm(real_HR[:,-1,:,:] - input_HR[:,-1,:,:] , p = 1, dim = [1,2])/self.resolution**2

        HR_error_l = torch.norm(self.downsample(output_HR[:,:,:,0] - input_HR[:,-1,:,:]), p =1 ,dim = [1,2])/self.obs_dim[0]**2
        Base_error_l = torch.norm( self.downsample(real_HR[:,-1,:,:] - input_HR[:,-1,:,:]), p = 1, dim = [1,2])/self.obs_dim[0]**2
        norm = torch.norm(input_HR[:,-1,:,:],p=1,dim=[1,2])/self.obs_dim[0]**2
        return HR_error, Base_error, HR_error_l , Base_error_l , norm

    def plot_baseline(self):
        HR_error, Base_error, HR_error_l , Base_error_l,norm  = self.test_trajectory(True)
        plt.figure()
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig('./img/error_of_traj_without_norm.png')
        wandb.log({"error_of_traj_without_norm": wandb.Image('./img/error_of_traj_without_norm.png')})

        HR_error, Base_error, HR_error_l , Base_error_l,norm  = self.test_trajectory(False)
        plt.figure()
        #t = np.linspace(0,1,90)
        plt.plot(t,HR_error.cpu().detach().numpy()/norm.cpu().detach().numpy(),label = 'HR_error')
        plt.plot(t,Base_error.cpu().detach().numpy()/norm.cpu().detach().numpy(),label = 'Base_error')
        plt.plot(t,HR_error_l.cpu().detach().numpy()/norm.cpu().detach().numpy(),label = 'HR_error_l')
        plt.plot(t,Base_error_l.cpu().detach().numpy()/norm.cpu().detach().numpy(),label = 'Base_error_l')
        plt.plot(t,norm.cpu().detach().numpy(),label = 'norm')
        plt.yscale('log')
        plt.legend()
        plt.savefig('./img/error_of_traj_with_norm.png')
        wandb.log({"error_of_traj_with_norm": wandb.Image('./img/error_of_traj_with_norm.png')})


    def test_util(self,data_real,data_pred,action,normal):
        '''
        ...
        '''
        error_data = torch.norm(self.downsample(data_real[1:,0,:,:] - data_pred[1:,0,:,:]), p =1 ,dim = [1,2])/self.obs_dim[0]**2
        # compute physic error
        # data_pred is time 1,2,3,4,...
        #action is action 0,1,2,3,...
        
        action_all = torch.zeros(data_real.shape[0]-1,self.resolution,self.resolution,2).to(self.device)
        action_all[:,:,:,0] = action[0:-1,:,:]
        action_all[:,:,:,1] = action[1:,:,:]
        action_all = action_all.permute(0,3,1,2) #length 2 res res
        data_input = torch.cat((data_pred[:-1],action_all),dim = 1).permute(0,2,3,1) #length ,res,res,3
        data_output = self.heat_forward(data_input) #length res res
        error_phy = torch.norm(data_output - data_real[1:,0,:,:],p = 1,dim = [1,2])
        error_HR =  torch.norm(data_real[1:,0,:,:] - data_pred[1:,0,:,:], p =1 ,dim = [1,2])/self.resolution**2
        norm_l = torch.norm(self.downsample(data_real[1:,0,:,:]),p = 1, dim = [1,2])/self.obs_dim[0]**2
        norm_h = torch.norm(data_real[1:,0,:,:],p = 1, dim = [1,2])/self.resolution**2 
        if normal:
            
            error_data = error_data / norm_l
            error_phy = error_phy / norm_h
            error_HR = error_HR/ norm_h
        return error_data,error_phy, error_HR,norm_l
    ...
    def test_6pic(self,test,normal):
        length =100
        if test == True:
            self.data_test = self.sample_traj(length)
        else: 
        ...
        input_HR = self.data_test.obs_buf[indexs,:,:].reshape(-1,self.n_step-1,self.resolution,self.resolution)
        #real_HR = self.data_test.obs_buf[indexs,:,:].reshape(-1,self.n_step-1,self.resolution,self.resolution) #batchsize , x,y
        output_HR = self.SFNO(self.downsample(input_HR).permute(0, 2, 3, 1)).permute(0,3,1,2) # batchsize ,x,y,1
        real_HR =input_HR[:,-1,:,:].unsqueeze(1)
        
        '''
        input is the action batch : (batch_size , n_step-1,act_dim)
        grid: (batch_size,n_step-1,nx,ny,2)
        '''
        action = self.data_test.act_buf[index_act].unsqueeze(1) #length ,1,act_dim
        grid_fine1 = self.get_grid([action.shape[0],1,self.resolution,self.resolution]).to(self.device)
        action = self.action_grid(action,grid_fine1).squeeze(1)      

        print('task1:','real_HR shape',real_HR.shape,'output_HR shape', output_HR.shape, 'action shape', action.shape )  
        error_data,error_phy, error_HR ,norm_l= self.test_util(real_HR,output_HR,action,normal)
        return error_data,error_phy,error_HR ,norm_l


    def plot_task1(self):

        error_data,error_phy,error_HR,norm_l = self.test_6pic(test = True,normal = True)
        plt.figure()
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig('./img/error_task1_test_normal.png')
        wandb.log({"error_task1_test_normal": wandb.Image('./img/error_task1_test_normal.png')})


        error_data,error_phy,error_HR,norm_l = self.test_6pic(test = False,normal = True)
        plt.figure()
        t = np.linspace(0,1,error_data.shape[0])
        plt.plot(t,error_data.cpu().detach().numpy(),label = 'error_data')
        plt.plot(t,error_phy.cpu().detach().numpy(),label = 'error_phy')
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig('./img/error_task1_normal.png')
        wandb.log({"error_task1_normal": wandb.Image('./img/error_task1_normal.png')})


        error_data,error_phy,error_HR,norm_l = self.test_6pic(test = True,normal = False)
        plt.figure()
        t = np.linspace(0,1,error_data.shape[0])
        plt.plot(t,error_data.cpu().detach().numpy(),label = 'error_data')
        plt.plot(t,error_phy.cpu().detach().numpy(),label = 'error_phy')
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig('./img/error_task1_test.png')
        wandb.log({"error_task1_test": wandb.Image('./img/error_task1_test.png')})


        error_data,error_phy,error_HR,norm_l = self.test_6pic(test = False,normal = False)
        plt.figure()
        t = np.linspace(0,1,error_data.shape[0])
        plt.plot(t,error_data.cpu().detach().numpy(),label = 'error_data')
        plt.plot(t,error_phy.cpu().detach().numpy(),label = 'error_phy')
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig('./img/error_task1.png')
        wandb.log({"error_task1": wandb.Image('./img/error_task1.png')})
