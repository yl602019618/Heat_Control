...
class MySolver:
    def __init__(self, geometry, function_space, params=None):
        ...
        self.u_t = Expression("exp(-x[1]*x[1]/2)",degree = 1)
        self.ys = np.linspace(self.geometry.min_y, self.geometry.max_y, 40)
        self.k = 1/16
    ...
    def get_data(self,t,action=None,result = None):
        ...
        if t ==0 : 
            ...
            f = Expression("a1+a2*x[0]+a3*x[1]+a4*x[0]*x[0]+a5*x[1]*x[1]+a6*x[0]*x[1]",degree=2, a1=0,a2=0,a3=0,a4=0,a5=0,a6=0)
    ...
    def init_solve(self):
        self.u_new = Function(self.V)
        self.u_target = Function(self.V)
        self.u_target.interpolate(self.u_t)
        self.solver = self.create_timestep_solver(self.u_new)
        self.u_new.interpolate(self.u_0)
        self.time = 0
    ...
    def get_reward(self):        
        out = np.zeros(self.ys.shape)
        for i in range(self.ys.shape[0]):
                xy = np.array([0.75,self.ys[i]])
                out[i] = (self.u_new(xy)-self.u_target(xy))**2
        return np.mean(out)
